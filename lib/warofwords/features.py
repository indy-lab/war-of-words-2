...
class Features:
    ...
    def get_group(self, group, by_feature_name=False):
        """Gets indices of a group.

        If `by_feature_name=True`, gets list of feature names.
        """
        if by_feature_name:
            return [self.get_name(idx) for idx in self._groups[group]]
        else:
